document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      // Get form fields
      const nameField = contactForm.querySelector('input[name="name"], input[type="text"]');
      ...
      if (messageField && (!messageField.value.trim() || messageField.value.trim().length < 10)) {
        ...
        const errorMsg = document.createElement('span');
        errorMsg.className = 'error-message';
        errorMsg.textContent = 'Please enter a message (at least 10 characters)';
        errorMsg.style.color = 'red';
        errorMsg.style.fontSize = '0.875rem';
        errorMsg.style.display = 'block';
        errorMsg.style.marginTop = '0.25rem';
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '3dfed444-ba6f-4d1f-99e3-614d22e4a907',
                    name: data.name || '',
                    email: data.email || '',
